...
Find-EXRRooms -Mailbox $MailboxName  -AccessToken $Token | foreach-object{
	...
	Get-EXRCalendarView -MailboxName $_.address -StartTime (Get-Date) -EndTime (Get-Date).AddDays(1) -AccessToken $Token  | foreach-object{
		$rptObj = "" | Select-Object Room,Organizer,Subject,Start,End
